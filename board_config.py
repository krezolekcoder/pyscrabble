TRIPLE_WORD_SCORE_COORDS = ((0, 0), (7, 0), (14, 0), 
                           (0, 7), (14, 7),
                           (0, 14), (7, 14), (14, 14))

DOUBLE_WORD_SCORE_COORDS = ((1, 1), (2, 2), (3, 3), (4, 4),
                            (13, 1), (12, 2), (11, 3), (10, 4),
                            (4, 10), (3, 11), (2, 12), (1, 13),
                            (13, 13), (12, 12), (11, 11), (10, 10))

TRIPLE_LETTER_SCORE_COORDS = ((5, 1), (9, 1),
                              (1, 5), (5, 5), (9, 5), (13,5),
                              (1, 9), (5, 9), (9, 9), (13,9),
                              (5, 13), (9, 13))

DOUBLE_LETTER_SCORE_COORDS = ((3, 0), (11, 0),
                              (6, 2), (8, 2), 
                              (0, 3), (7, 3), (14, 3),
                              (2, 6), (6, 6), (8, 6), (12, 6), 
                              (3, 7), (11, 7),
                              (2, 8), (6, 8), (8, 8), (12, 8),
                              (0, 11), (7, 11), (14, 11),
                              (6, 12), (8, 12),
                              (3, 14), (11, 14))

TILE_DL_COLOR = (173, 216, 230)   # Light Blue (Sky Blue)
TILE_TL_COLOR =  (70, 130, 180)    # Deep Blue (Navy Blue)
TILE_DW_COLOR = (255, 182, 193)     # Light Red (Light Pink)
TILE_TW_COLOR = (220, 20, 60)          # Deep Red (Dark Red)
TILE_DEFAULT_COLOR = (255, 255, 255) # white 

WHITE_COLOR = (255, 255, 255)
BLACK_COLOR = (0, 0, 0)

WINDOW_WIDTH = 900
WINDOW_HEIGHT = 1000
BOARD_WIDTH = 900
BOARD_HEIGHT = 900
BOARD_TILE_CNT = 15 
TILE_SIZE = BOARD_WIDTH / BOARD_TILE_CNT

PLAYER_LETTERS_START_Y_COORD = 920
PLAYER_LETTERS_START_X_COORD = 40

LETTERS_Y_OFFSET = PLAYER_LETTERS_START_Y_COORD - BOARD_HEIGHT
LETTERS_X_OFFSET = PLAYER_LETTERS_START_X_COORD

FONT_SIZE = 60

HEADING_LEFT = (-1, 0)
HEADING_RIGHT = (1, 0)
HEADING_UP = (0, -1)
HEADING_DOWN = (0, 1)

PLAYER_LETTERS_OFFSET = 10

BUTTON_GREEN_COORDS = (750, BOARD_HEIGHT + LETTERS_Y_OFFSET)
BUTTON_RED_COORDS = (810, BOARD_HEIGHT + LETTERS_Y_OFFSET)

BUTTON_GREEN = (12, 15)
BUTTON_RED = (13, 15)